...
async def check_ai_connection() -> bool:
    ...
    try:
        ...
        if result.get("success"):
            logger.info(f"Azure OpenAI connected: {result.get('response', 'OK')}")
            return True
        else:
            logger.warning(f"Azure OpenAI check failed: {result.get('error', 'Unknown error')}")
            return False
    except Exception as e:
...
async def auto_setup() -> None:
    ...
    logger.info("Step 4: Checking Azure OpenAI connection...")
